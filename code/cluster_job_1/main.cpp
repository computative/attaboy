...
int main(int argc, char *argv[])
{
    ...
    //int n = 20;
    double thd = 8;
    double dt = (Tstop - Tstart)/thd;
    int m = n;
    int timesteps = thd;
    int samplepoint = 100000;
    double * beta = new double [timesteps];
    for (int i = 0; i < timesteps; i++) {
        beta[i] = 1/(Tstart + i*dt);
    }
    ...
    # pragma omp parallel num_threads(8)
    {
        ...
        for (int l = id; l < timesteps; l=l+threads) {
            ...
            delete [] A;
            # pragma omp critical
            cout << n << ' ' << 1/beta[l] << ' ' << avg[0]/(N - samplepoint) << ' ' << avg[2]/(N - samplepoint) << ' '<< (avg[1]/(N - samplepoint) - pow(avg[0]/(N - samplepoint),2))*beta[l]*beta[l] << ' ' << (avg[3]/(N - samplepoint) - pow(avg[2]/(N - samplepoint),2))*beta[l] << endl;
        }
    }
}
