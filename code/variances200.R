...
################
data = read.table("../resources/60_2T0.txt",col.names=c("n","E"))
ns = as.numeric(names(table(data$n)))
tmp = rep(0,length(ns))
tmp2 = rep(0,length(ns))
target = data[data$n > 100000,]$E
...
data = read.table("../resources/100_2T0.txt",col.names=c("n","E"))
ns = as.numeric(names(table(data$n)))
tmp = rep(0,length(ns))
tmp2 = rep(0,length(ns))
target = data[data$n > 500000,]$E
g = 24
cats = seq(min(target) , max(target), (max(target) - min(target) )/g )
groups = cut(target, breaks = g)
P = as.numeric(table( groups ))/length(target)
...
data = read.table("../resources/140_2T0.txt",col.names=c("n","E"))
ns = as.numeric(names(table(data$n)))
tmp = rep(0,length(ns))
tmp2 = rep(0,length(ns))
target = data[data$n > 100000,]$E
